...
def get_geometry(arguments: dict):
    ...
    if "intersects" in arguments and "bbox" in arguments:
        raise ValidationError('Only one of bbox and intersects can be used at a time.')

    if "bbox" in arguments and arguments['bbox'] is not None:
        ...
        if len(request_bbox) == 4:
            ...
            raise ValidationError(f"invalid bbox length ({len(request_bbox)}) for bbox {request_bbox}")
